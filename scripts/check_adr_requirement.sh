...
if ! echo "${CHANGED}" | rg -q \
  "^(packages/engine/src/ralphite_engine/(headless_agent|orchestrator)\.py|apps/tui/src/ralphite_tui/cli\.py|packages/schemas/json/plan-spec-v5\.schema\.json|packages/schemas/python/src/ralphite_schemas/plan_v5\.py)$"; then
