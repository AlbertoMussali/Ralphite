...
def _run(args: list[str], *, text: str | None = None, shell: bool = False) -> ActionResult:
    try:
...
def copy_text_to_clipboard(text: str) -> ActionResult:
    ...
    if sys.platform == "darwin":
        if shutil.which("pbcopy"):
            ...
            return ActionResult(ok=result.ok, message="Copied to clipboard." if result.ok else result.message, command=result.command)
    ...
    if shutil.which("xclip"):
        ...
        return ActionResult(ok=result.ok, message="Copied to clipboard." if result.ok else result.message, command=result.command)
    return ActionResult(ok=False, message="No clipboard utility found (expected wl-copy or xclip).")
...
def open_local_path(path: Path) -> ActionResult:
    ...
    if sys.platform == "darwin":
        ...
        return ActionResult(ok=result.ok, message="Opened path." if result.ok else result.message, command=result.command)
