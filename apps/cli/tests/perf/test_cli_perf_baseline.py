...
def test_cli_perf_within_10_percent_baseline(tmp_path: Path) -> None:
    ...
    baseline_path = root / "benchmarks" / "baseline" / "pre_pivot_cli.json"
    output_path = tmp_path / "current.json"

    completed = subprocess.run(
        ["python3", "scripts/bench_cli.py", "--repeats", "1", "--output", str(output_path)],
        cwd=root,
        check=False,
        capture_output=True,
        text=True,
    ...
    for key in ("quickstart", "check_strict", "run", "cli_tests"):
        ...
        assert cur_seconds <= base_seconds * 1.10, f"{key} runtime regression: {cur_seconds} > {base_seconds * 1.10}"
        ...
        if isinstance(base_rss, (int, float)) and isinstance(cur_rss, (int, float)):
            assert cur_rss <= base_rss * 1.10, f"{key} RSS regression: {cur_rss} > {base_rss * 1.10}"
