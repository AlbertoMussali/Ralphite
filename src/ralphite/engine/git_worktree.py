...
class GitWorktreeManager:
    ...
    def _cleanup_stale_managed_worktree(
        ...
        removed, remove_messages = self._remove_managed_worktree_path(
    ...
    def _normalize_rel_path(self, raw_path: str | Path) -> str:
        ...
        if candidate.is_absolute():
            try:
                candidate = candidate.resolve().relative_to(self.workspace_root)
            except Exception:  # noqa: BLE001
                return ""
    ...
    def _normalize_rel_paths(self, raw_paths: list[str] | tuple[str, ...]) -> set[str]:
        return {
            ...
            if (normalized := self._normalize_rel_path(item))
        }
    ...
    def _delete_tree_with_retry(
        ...
        kind = self._classify_delete_failure(path, last_detail)
    ...
    def _remove_managed_worktree_path(
        ...
        if not path.is_relative_to(self._worktrees_root()):
            return False, [f"refusing to remove non-managed worktree path {path}"]
        return self._delete_tree_with_retry(path, branch=branch, stale=stale)
    ...
    def _auto_resolve_conflict_file(self, path: Path) -> tuple[bool, dict[str, Any]]:
        ...
        chunks, had_conflict = self._merge_conflict_blocks(original)
        if not had_conflict:
        ...
        for plain, ours, theirs in chunks:
            if not ours and not theirs:
                ...
                continue
            resolver_kind = self._conflict_resolver_kind(path, ours, theirs)
            if not resolver_kind:
                return False, {"error": "unsupported conflict shape"}
            resolver_kinds.add(resolver_kind)
            resolved_parts.extend(self._merge_unique_lines(ours, theirs))
    ...
    def _attempt_auto_resolve_merge_conflicts(
        ...
        conflict_files = self._tracked_unmerged_files(
            worktree
        ) or self._collect_conflict_files(worktree)
        ...
        for rel_path in conflict_files:
            ...
            resolved, details = self._auto_resolve_conflict_file(file_path)
            if not resolved:
                unsupported_files.append(rel_path)
                continue
            add = self._git(["add", "--", rel_path], cwd=worktree, check=False)
            if add.returncode != 0:
                unsupported_files.append(rel_path)
                continue
        ...
        commit = self._git(["commit", "--no-edit"], cwd=worktree, check=False)
    ...
    def pre_base_integration_check(
        ...
        ignored = self._normalize_rel_paths(list(ignore_paths or []))
    ...
    def prepare_phase_integration(
        ...
        for branch in worker_branches:
            ...
            if merged.returncode != 0:
                ...
                resolved, resolve_meta = self._attempt_auto_resolve_merge_conflicts(
                    integration_path
                )
                if resolved:
    ...
    def integrate_phase(
        ...
        *,
        ...
        precheck = self.pre_base_integration_check(
    ...
    def cleanup_orphaned_run_artifacts(self, run_id: str | None = None) -> list[str]:
        ...
        for item in inventory.get("worktrees", []):
            ...
            if not path_obj.exists():
                ...
                continue
            _removed, remove_messages = self._remove_managed_worktree_path(
