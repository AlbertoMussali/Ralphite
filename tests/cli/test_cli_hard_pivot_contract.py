...
from ralphite.cli.checks.suites import STRICT_SUITES
...
def test_strict_suites_have_no_legacy_paths() -> None:
    for _name, command in STRICT_SUITES:
        joined = " ".join(command)
        assert "tests/cli" in joined or "tests/engine" in joined
...
def test_check_full_targets_cli_tests(tmp_path: Path, monkeypatch) -> None:  # type: ignore[no-untyped-def]
    ...
    assert any("tests/cli" in row for row in joined)
