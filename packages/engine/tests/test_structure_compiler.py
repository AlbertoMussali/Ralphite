...
def test_compile_execution_structure_enforces_lane_barriers() -> None:
    plan = PlanSpecV2.model_validate(
        {
            "version": 2,
            "plan_id": "lane_barrier",
            "name": "Lane Barrier",
            "task_source": {"kind": "markdown_checklist", "path": "RALPHEX_TASK.md", "parser_version": 2},
            "agent_profiles": [
    ...
    tasks, parse_issues = parse_task_lines(
        [
            ...
            "- [ ] Exec A <!-- id:t2 phase:phase-1 lane:parallel deps:t1 agent_profile:worker_default -->",
            "- [ ] Exec B <!-- id:t3 phase:phase-1 lane:parallel deps:t1 agent_profile:worker_default -->",
    ...
    assert levels["phase-1::parallel::t2"] < levels["phase-1::seq_post::t4"]
    assert levels["phase-1::parallel::t3"] < levels["phase-1::seq_post::t4"]
